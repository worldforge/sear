...
#include "../src/ObjectProperties.h"
...
Models *Impostor_Loader::loadModel(WorldEntity *we, ObjectProperties *op, const std::string &file_name) {
  ...
  int id = GL_Render::instance()->requestTexture(std::string("impostor_") + type);
  if (id == -1) {
    type = we->parent();
    id = GL_Render::instance()->requestTexture(std::string("impostor_") + type);
  }
  if (id == -1) {
