...
// $Id: Bindings.cpp,v 1.1.2.1 2003-01-05 14:22:01 simon Exp $
...
void Bindings::loadBindings(const std::string &file_name) {
  ...
  if (_bindings) { 
    if (!_bindings->readFromFile(file_name)) {
      std::cerr << "Error processing " << file_name << std::endl;
    }
  }
...
std::string Bindings::getBinding(const std::string &key) {
  ...
  if (!_bindings) {
    std::cerr << "Bindings: Error - bindings config object not created" << std::endl;
    return "";
  }
  if (key.empty()) return "";
...
void Bindings::registerCommands(lua_State *L) {
  lua_register(L, "bind", l_bind);
}

int Bindings::l_bind(lua_State *L) {
  // Get stack top	
  int n = lua_gettop(L);
  ...
  if (n == 2) {
    // Get the key
    key = lua_tostring(L, n-1);
    // Get its binding
    binding = lua_tostring(L, n);
  } else {
    // Get key
    key == lua_tostring(L, n);
    // Set binding to nothing
    binding = "";
  }
  if (debug) std::cout << "Binding " << key << " to " << binding << std::endl;
  // bind binding to key
  bind(key, binding);
  return 0;
}
