...
## Modified script to build a cross compiler from the SDL web site
...
MINGW_URL=http://kent.dl.sourceforge.net/sourceforge/mingw
GCC_VERSION=3.4.5-20060117-1
GCC=gcc-$GCC_VERSION
GCC_CORE=gcc-core-$GCC_VERSION
GCC_CORE_ARCHIVE=$GCC_CORE-src.tar.gz
GCC_GXX=gcc-g++-$GCC_VERSION
GCC_GXX_ARCHIVE=$GCC_GXX-src.tar.gz
GCC_G77=gcc-g77-$GCC_VERSION
GCC_G77_ARCHIVE=$GCC_G77-src.tar.gz
GCC_OBJC=gcc-objc-$GCC_VERSION
GCC_OBJC_ARCHIVE=$GCC_OBJC-src.tar.gz
GCC_PATCH=""
BINUTILS=binutils-2.17.50-20060824-1
...
download_files()
{
	...
	download_file "$GCC_GXX_ARCHIVE" "$MINGW_URL"
...
configure_binutils()
{
	...
	"$SRCDIR/$BINUTILS-src/configure" --prefix="$PREFIX" --target=$TARGET &> configure.log
	cd "$TOPDIR"
}
...
extract_gcc()
{
	...
	gzip -dc "$SRCDIR/$GCC_GXX_ARCHIVE" | tar xf -
	gzip -dc "$SRCDIR/$GCC_G77_ARCHIVE" | tar xf -
	gzip -dc "$SRCDIR/$GCC_OBJC_ARCHIVE" | tar xf -
	cd "$TOPDIR"
}
...
configure_gcc()
{
	...
	"$SRCDIR/$GCC/configure" -v \
		...
		--with-gcc --enable-languages=c,c++,f77,objc \
...
build_gcc()
{
	...
	echo "Building gcc"
	make $JOBS LANGUAGES="c c++ f77 objc" &> make.log
	if test $? -ne 0; then
...
install_gcc()
{
	cd "$TOPDIR/gcc-$TARGET"
	echo "Installing gcc"
	make LANGUAGES="c c++ f77 objc" install &> make-install.log
	if test $? -ne 0; then
