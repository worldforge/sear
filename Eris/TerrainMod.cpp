...
bool InnerTerrainModCrater::parseAtlasData(const Atlas::Message::MapType& modElement)
{
	...
	if (shapeMap) {
		if (shapeType == "ball") {
			...
			// TODO SG: Make this a abstract method on the parent class
			//pos.z() = EmberOgre::getSingleton().getTerrainGenerator()->getHeight(TerrainPosition(pos.x(), pos.y()));
			pos.z() = mTerrainMod.mTMH->getZPos(pos.x(), pos.y());
	...
	mTerrainMod.mTMH->logFailure("Crater terrain mod defined with incorrect shape");
	return false;
}
...
bool InnerTerrainModSlope::parseAtlasData(const Atlas::Message::MapType& modElement)
{
	...
	mTerrainMod.mTMH->logFailure("SlopeTerrainMod defined with incorrect shape");
	return false;
}
...
bool InnerTerrainModLevel::parseAtlasData(const Atlas::Message::MapType& modElement)
{
	...
	mTerrainMod.mTMH->logFailure("Level terrain mod defined with incorrect shape");
	return false;
}
...
bool InnerTerrainModAdjust::parseAtlasData(const Atlas::Message::MapType& modElement)
{
	...
	mTerrainMod.mTMH->logFailure("Adjust terrain mod defined with incorrect shape");
	return false;
}
...
TerrainMod::TerrainMod(Entity *entity, TerrainModHandler *tmh)
: mEntity(entity)
, mInnerMod(0)
, mTMH(tmh)
{
...
bool TerrainMod::parseMod()
{
	...
	// We now create a TerrainMod for every entity 
	//	mTMH->logFailure("TerrainMod defined on entity with no terrainmod attribute");
		return false;
	}
	...
	if (!modifier.isMap()) {
		mTMH->logFailure("Terrain modifier is not a map");
		return false;
	}
